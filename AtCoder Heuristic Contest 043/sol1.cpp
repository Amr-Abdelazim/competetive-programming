...
struct process
{
    ...
    void bld(pii a, pii b, pii c)
    {
        ...
        {
            if (mony < COST_STATION - COST_RAIL)
            {
                actions.push_back({-1, -1, -1});
                running = false;
                return;
...
signed main()
{
    ...
    {
        return grid.cel[x.first][x.second] == STATION && grid.cel[y.first][y.second] == STATION && grid.is_same(x.first, x.second, y.first, y.second);
    };
    auto collect_mony = [&]()
    {
        rep(i, m)
    ...
    while (q--)
    {
        ...
        if (processes.empty())
        {
            while (cur < m && is_connected(home[ord[cur]], work[ord[cur]]) || cur < m && distance(home[ord[cur]], work[ord[cur]]) * COST_RAIL >= mony - 2 * COST_STATION)
            {
                cur++;
            }
            if (cur < m)
